...
int main() {
    ...
    dosyadan_stokal(stoklar, &kayitli_urun_sayisi); //ampersant ile gönderme sebebi fonksiyonu void ile yazmamız dosyadan stok aldığı için kayıtlı ürün sayısı artıyor.
    int seçim;
    do { // while içine yazdığımız koşulu sağladığı sürece kod çalışır. sağlamazsa döngü kırılır.
        ...
        switch (seçim) {
            case 1:
                urun_ekle(stoklar, &kayitli_urun_sayisi); // ürün ekle fonksiyonuna stoklar veri yapısını ve kayıtlı ürün sayısının adresini yolladık.
                break;
            case 2:
            ...
            case 4:
                stok_goruntule(stoklar, kayitli_urun_sayisi);
                stok_sil(stoklar,&kayitli_urun_sayisi); // & ile yollama sebebimiz ürün bir eksiliyor.
                break;
            case 5:
